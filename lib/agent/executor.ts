...
"nodes": [ ..., { "name": "AI Classifier", "type": "magicflux-nodes.aiClassifier", "parameters": { "outputField": "classification", ... } }, { "name": "Needs Review?", "type": "n8n-nodes-base.if", "parameters": { "conditions": { "boolean": [ { "value1": "={{$json[\"needs_review\"]}}", "operation": "equal", "value2": true } ] } } }, { "name": "Human Review", "type": "magicflux-nodes.humanReview", "parameters": { "outputField": "classification", ... } }, ... ]
...
HUMAN REVIEW CONTRACT -- MANDATORY whenever this automation needs to pause for a real person to approve, reject, or otherwise decide an outcome (e.g. "flag for human review instead of guessing", "require approval before proceeding", "escalate risky/uncertain cases to a human", refund/order/content approval): you MUST insert a real node with type EXACTLY "magicflux-nodes.humanReview". This is a genuinely new, real, supported node type in this product (like magicflux-nodes.aiClassifier) -- NEVER represent "human review"/"approval" using n8n-nodes-base.set (it is a data no-op, not a pause) or n8n-nodes-base.if alone (it only reads a value, it creates no durable record a human can act on and does not actually pause anything). The humanReview node's "parameters" must include:
  ...
  - "outputField": REQUIRED whenever allowedOutcomes is reviewing an upstream aiClassifier's classification (see HUMAN DECISION AUTHORITY CONTRACT below for exactly when) -- set it to that classifier's own "outputField" value. Omit entirely for a plain approval/reject decision unrelated to any classifier.
...
HUMAN DECISION AUTHORITY CONTRACT -- MANDATORY, Phase 9.9.3.2/9.9.4C. A human's decision at a magicflux-nodes.humanReview node is FINAL and must never be second-guessed by re-reading an AI classifier's earlier, now-superseded output -- and must become the value ANY downstream reader (routing, an Airtable/database field mapping, a message template) sees as canonical, not just whichever branch happens to fire. Concretely: once execution passes through Human Review, NOTHING downstream may branch again on the same field the AI classifier originally wrote (e.g. "={{$json[\"classification\"]}}") to decide which of the outcome's own actions to run -- Human Review's own output ports already encode exactly which outcome the human chose (main[0] for the first entry in "allowedOutcomes", main[1] for the second, etc., routed deterministically by node id, not by re-evaluating any data field).
Whenever a humanReview node's "allowedOutcomes" are exactly the SAME set of labels an upstream aiClassifier's "allowedLabels" can produce (i.e. this Human Review is genuinely reviewing that classification, not an unrelated approve/reject decision), it MUST also set "outputField": "<the classifier's own outputField, e.g. \"classification\">" on its own parameters -- mirroring aiClassifier's own "outputField" parameter name and meaning. On resume, the runtime deterministically overwrites exactly that one field with the human's chosen outcome string, never any other field, so every downstream reader (an Airtable "Classification"/"Confidence"-style mapping included) reflects the human's decision rather than the AI's original, possibly-overridden value. Set this REGARDLESS of which routing topology below is used -- it costs nothing when nothing downstream happens to read that field, and fixes real data otherwise.
Two acceptable ROUTING shapes, in order of preference (independent of the outputField requirement above):
  1. PREFERRED -- when allowedOutcomes names the SAME set of routing destinations the confident (non-reviewed) path already uses (e.g. allowedOutcomes: ["Hot","Warm","Cold"] mirroring the classifier's own allowedLabels), wire each of Human Review's outcome ports DIRECTLY to that outcome's terminal action node(s) -- the exact same node names the confident path's own "If Hot"/"If Warm"/... chain already targets. Do NOT insert a second "If Hot"/"If Warm"/"If Cold"-style node after Human Review to re-check the classification -- that node would read the classifier's ORIGINAL field, which Human Review only overwrites via outputField, so an unconfigured re-check would silently produce ZERO downstream actions on disagreement (the re-check node's own condition would fail and its false branch is typically empty). The confident (non-reviewed) path may still use its own "If Hot" -> "If Warm" -> ... chain unchanged -- that path never went through Human Review and legitimately reads the classifier's real, current value.
  2. ALTERNATIVE -- only if the request genuinely requires Human Review to rejoin the SAME classification-routing chain the confident path uses (rather than route directly): rely on the "outputField" set above -- a downstream IF re-reading that field on resume will see the human's decision, not the stale AI value. Only use this when explicitly rejoining a shared chain; the PREFERRED direct-port shape above still needs "outputField" set (for downstream data correctness) but never a re-check node.
Never leave Human Review's outcome ports feeding a node that re-checks the original AI field without a matching "outputField" configured -- that graph will be rejected before persistence.

SLA ACKNOWLEDGMENT CONTRACT -- MANDATORY whenever this automation needs a real person to take OWNERSHIP of an already-decided outcome (e.g. a Hot lead, an urgent ticket, a flagged order) within a business-defined deadline, escalating if nobody does (e.g. "notify the team and escalate if nobody responds within 15 minutes", "make sure someone acknowledges this lead"): insert a real node with type EXACTLY "magicflux-nodes.waitForAcknowledgment" AFTER the notification action(s) it covers (e.g. after Airtable/Gmail/Slack for that outcome), never instead of them -- a notification proves a message was delivered, never that a human took ownership; this node is what actually proves the latter. Do NOT represent this with a plain Wait node (a Wait node has no acknowledgment concept at all, it only ever proceeds after its delay) and do NOT represent it with magicflux-nodes.humanReview (that node answers "what should the AI have decided instead", has no deadline/timer at all by design, and its outcomes are a review DECISION, not an ownership acknowledgment -- conflating the two produces a workflow that claims a capability neither node actually has). Parameters must include:
  - "slaMinutes": the business's own configured SLA duration in minutes, a positive number -- NEVER a hard-coded default like 15; use exactly the duration the request states, or ask if none was given. Do not invent one.
  - "outputField": optional (defaults to "acknowledgment_status") -- the field downstream nodes/templates can read ("acknowledged" or "timed_out").
...
Use real n8n node types (e.g. n8n-nodes-base.gmailTrigger, n8n-nodes-base.openAi, etc.), EXCEPT for the four MagicFlux-native capabilities: "magicflux-nodes.aiClassifier" (see AI CLASSIFICATION CONTRACT above), "magicflux-nodes.humanReview" (see HUMAN REVIEW CONTRACT above), "magicflux-nodes.waitForAcknowledgment" (see SLA ACKNOWLEDGMENT CONTRACT above), and "magicflux-nodes.createAcknowledgmentChallenge" (see ACKNOWLEDGMENT URL AVAILABILITY above) -- all four are genuine, real, supported node types in this product, not n8n nodes, and must be spelled EXACTLY that way when used.
Each node must have: id, name, type, typeVersion, position [x,y], parameters, displayName, provider.
For provider use ONLY these canonical ids: stripe, airtable, openai, slack, gmail, google_drive, google_sheets, telegram, shopify, hubspot, elevenlabs, claude, facebook, canva, twitter, whatsapp, cloudflare_ai, deepgram, supabase. n8n-nodes-base.if, n8n-nodes-base.set, magicflux-nodes.aiClassifier, magicflux-nodes.humanReview, magicflux-nodes.waitForAcknowledgment, and magicflux-nodes.createAcknowledgmentChallenge need NO provider at all (leave provider null) -- they are internal/platform-native nodes, never an external system.
  ...
  try {
    ...
    switch (toolName) {
      ...
      case 'generate_workflow_json': {
        ...
        // Phase 9.9.3.2/9.9.4C -- product-truth backstop: a human's decision
        // at a Human Review node must be authoritative, for BOTH routing
        // (never re-check the AI's original field) and downstream DATA
        // (a genuine classification review must configure "outputField" so
        // the human's choice becomes canonical, e.g. for an Airtable
        // mapping) -- see HUMAN DECISION AUTHORITY CONTRACT above.
