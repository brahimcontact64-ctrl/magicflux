...
Use real n8n node types (e.g. n8n-nodes-base.gmailTrigger, n8n-nodes-base.openAi, etc.), EXCEPT for the three MagicFlux-native capabilities: "magicflux-nodes.aiClassifier" (see AI CLASSIFICATION CONTRACT above), "magicflux-nodes.humanReview" (see HUMAN REVIEW CONTRACT above), and "magicflux-nodes.waitForAcknowledgment" (see SLA ACKNOWLEDGMENT CONTRACT above) -- all three are genuine, real, supported node types in this product, not n8n nodes, and must be spelled EXACTLY that way when used.
Each node must have: id, name, type, typeVersion, position [x,y], parameters, displayName, provider.
For provider use ONLY these canonical ids: stripe, airtable, openai, slack, gmail, google_drive, google_sheets, telegram, shopify, hubspot, elevenlabs, claude, facebook, canva, twitter, whatsapp, cloudflare_ai, deepgram, supabase. n8n-nodes-base.if, n8n-nodes-base.set, magicflux-nodes.aiClassifier, magicflux-nodes.humanReview, and magicflux-nodes.waitForAcknowledgment need NO provider at all (leave provider null) -- they are internal/platform-native nodes, never an external system.
