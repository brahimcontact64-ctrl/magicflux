...
describe('V1 — frozenMap: all standard read operations work correctly', () => {
  ...
  it('V1.5: .size is 19', () => {
    expect(HANDLER_NODE_ALLOWLIST.size).toBe(19);
  });

  it('V1.6: .entries() iterates all 19 pairs', () => {
  ...
  it('V1.7: .keys() iterates all 19 node types', () => {
    const keys = [...HANDLER_NODE_ALLOWLIST.keys()];
    expect(keys).toHaveLength(19);
    expect(keys.every(k => typeof k === 'string')).toBe(true);
  });

  it('V1.8: .values() iterates all 19 handlers', () => {
  ...
  it('V1.10: .forEach() visits all 19 entries', () => {
    let count = 0;
