...
describe('V1 — frozenMap: all standard read operations work correctly', () => {
  ...
  it('V1.5: .size is 18', () => {
    expect(HANDLER_NODE_ALLOWLIST.size).toBe(18);
  });

  it('V1.6: .entries() iterates all 18 pairs', () => {
  ...
  it('V1.7: .keys() iterates all 18 node types', () => {
    const keys = [...HANDLER_NODE_ALLOWLIST.keys()];
    expect(keys).toHaveLength(18);
    expect(keys.every(k => typeof k === 'string')).toBe(true);
  });

  it('V1.8: .values() iterates all 18 handlers', () => {
  ...
  it('V1.10: .forEach() visits all 18 entries', () => {
    let count = 0;
